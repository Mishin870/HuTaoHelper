...
public partial class MainWindow {
	...
	private async void NotificationTargets_OnClick(object sender, RoutedEventArgs e) {
		await NotificationsVisualController.ShowTargetsSelector();
	}

	private async void AccountChangeNotificationTargetMenu_OnClick(object sender, RoutedEventArgs e) {
		await DoAccountContextAction(e, async account => {
			await NotificationsVisualController.ShowTargetsSelector((code, target) => {
				account.NotificationTarget = code;
				Settings.Save();
			});
		}, false);
	}
