...
namespace HuTaoHelper.Web {
	...
	public class ApiClient {
		...
		public async Task<T> GetExecuteRequest<T>(string path, Action<ApiHttpRequestMessage>? postProcess = null) {
			var req = new Uri($"{path}");
			return await ExecuteRequest<T>(req, HttpMethod.Get, null, postProcess);
		}

		public async Task<T> PostExecuteRequest<T>(string path, JsonContent? jsonContent = null, 
			Action<ApiHttpRequestMessage>? postProcess = null) {
			var req = new Uri($"{path}");
			return await ExecuteRequest<T>(req, HttpMethod.Post, jsonContent, postProcess);
		}
